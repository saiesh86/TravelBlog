    ...
    public static class CssMinify
    {
        ...
        public static string Minify(string css, int columnWidth)
        {
            ...
            */
            ...
            // 2013-03-06 JA removed the "(" character from the below line, original value is above in comment
            // removing preceeding space before ( was causing invalid and broken css in some cases
            // as reported here https://www.mojoportal.com/Forums/Thread.aspx?pageid=5&t=11428~1
            css = Regex.Replace(css, @"(?#no preceding space needed)\s+((?:[!{};>+)\],])|(?<={[^{}]*):(?=[^}]*}))", "$1");
        ...
        {
            if (shortColorNames.Count > 0) { return; }
            ...
            if (shortHexColors.Count > 0) { return; }
...
(?:(?<property>position)\s*:\s*(?<unit>((?<horizontal>left | center | right|(?:0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex)))))\s+(?<vertical>top | center | bottom |(?:0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex))))))|
    ...
    // the commented version (newer from the original author) below caused an error on deployment so commented out, the above one is proven stable after long use

//    public static class CssMinifyv2
//    {
//        public static Hashtable shortColorNames = new Hashtable();
//        public static Hashtable shortHexColors = new Hashtable();
//        public static string Minify(string css)
...
//               But, be careful not to turn "p :link {...}" into "p:link{...}"
//            */
//            // the line below was causing css parse error because it changed this:
//            //@media handheld, only screen and (max-width: 767px) { .reveal-modal-bg { position: absolute; } 
//            // to this:
//            //@media handheld, only screen and(max-width: 767px) { .reveal-modal-bg { position: absolute; } 
//            // the space after "and" is needed
